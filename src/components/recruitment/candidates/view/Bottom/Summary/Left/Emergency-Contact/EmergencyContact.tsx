...
export default function EmergencyContact ({ data }: Props) {
    ...
    <>
      <div className='flex-1'>
        ...
        <section className='p-3 py-5 w-full border-2 border-t-0 flex  p-2 flex-col gap-2'>
          {data.candidateReferences.length > 0 &&
            data.candidateReferences.map((e: any, i: number) => (
              <div className='border relative p-1' key={i}>
                <BsThreeDotsVertical className='absolute right-2 top-1 text-[#1273eb] w-5 h-5' />
                <EachExperience name={'Name'} value={e.referenceName} />
                <EachExperience name={'Relationship'} value={e.relation} />
                <EachExperience name={'Mobile Number'} value={e.phoneNumber} />
                <EachExperience name={'Email'} value={e.emailAddress} />
              </div>
            ))}
        </section>
        {data.candidateReferences.length === 0 && (
          <section className='p-3 py-5 w-full border-2 border-t-0 flex justify-between items-center p-5 flex-col gap-2'>
            ...
            <button
              onClick={() => {
                setModalState({ isOpen: true, type: 'emergency' })
              }}
